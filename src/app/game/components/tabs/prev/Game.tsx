...
const Game = ({ id, team, opponent, teamHome, homeScore, awayScore, leagueName, homeShootoutScore, awayShootoutScore, gameResult, gameDate }: Props) => {
    ...
    return (
        <Link
            style={{ borderColor: getResultColor(gameResult) }}
            href={`/game/${id}`}
        ...
        >
            ...
            <div className='flex flex-row items-center justify-between  px-2 py-1'>
                ...
                <div className='text-[10px]  text-gray-400'>{gameDate.split("T")[0]}</div>
            </div>
            {
                [home, away].map((team: any, i: number) => (
                    ...
                    <div className='flex flex-row justify-between items-center'>
                        ...
                        <div className={`text-center bg-[--tw-color-950] text-lg  py-1 ${i === 1 ? "rounded-br-lg" : ""} w-[50px] font-semibold`}>{i === 0 ? homeScore : awayScore}</div>
                    </div>
